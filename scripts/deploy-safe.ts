...
class DeploymentContext {
  ...
  constructor(network: string) {
    ...
    if (!NETWORK_CONFIGS[network]) {
      throw new Error(
        `Unsupported network: ${network}. Supported: ${Object.keys(NETWORK_CONFIGS).join(', ')}`
      );
    }
  ...
  async validate(): Promise<void> {
    ...
    if (network.chainId !== this.networkConfig.chainId) {
      throw new Error(
        `Chain ID mismatch: expected ${this.networkConfig.chainId}, got ${network.chainId}`
      );
    }
    ...
    if (balance.lt(minBalance)) {
      throw new Error(
        `Insufficient balance: ${ethers.utils.formatEther(balance)} ETH (minimum ${ethers.utils.formatEther(minBalance)} ETH)`
      );
    }
    ...
    const artifactPath = path.join(
      __dirname,
      '../artifacts/contracts/JitExecutor.sol/JitExecutor.json'
    );
    if (!fs.existsSync(artifactPath)) {
      throw new Error(
        `Contract artifact not found: ${artifactPath}. Run 'npm run build:contracts' first.`
      );
    }
  ...
  async validatePositionManager(): Promise<void> {
    try {
      ...
      if (code === '0x') {
        throw new Error(
          `Position manager contract not found at ${this.config.positionManager}`
        );
      }
  ...
  displaySummary(): void {
    ...
    console.log(
      `💰 Deployer Balance:     ${ethers.utils.formatEther(this.wallet.getBalance())} ETH`
    );
    console.log(
      `🔧 Mode:                 ${this.dryRun ? 'DRY RUN (validation only)' : 'LIVE DEPLOYMENT'}`
    );
    console.log('\n📊 Configuration');
    console.log(
      '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
    ...
    console.log(
      `💎 Min Profit Threshold: ${this.config.minProfitThreshold} ETH`
    );
...
async function deployContract(ctx: DeploymentContext): Promise<{
  ...
  console.log(
    `   minProfitThreshold: ${ethers.utils.formatEther(minProfitThreshold)} ETH`
  );
  console.log(
    `   maxLoanSize:        ${ethers.utils.formatEther(maxLoanSize)} ETH`
  );
  ...
  const contract = await contractFactory.deploy(
    ...
    positionManager,
    {
      gasPrice: ctx.networkConfig.gasPrice
        ? ethers.BigNumber.from(ctx.networkConfig.gasPrice)
        : undefined,
    }
  );
  ...
  const receipt = await contract.deployTransaction.wait(
    ctx.networkConfig.confirmationBlocks
  );
  ...
  console.log(
    `💸 Deployment Cost:       ${ethers.utils.formatEther(deploymentCost)} ETH`
  );
...
async function verifyDeployment(
  ...
  try {
    ...
    console.log(
      `   Min Profit Threshold: ${ethers.utils.formatEther(minProfit)} ETH`
    );
    console.log(
      `   Max Loan Size:        ${ethers.utils.formatEther(maxLoan)} ETH`
    );
    ...
    const expectedMinProfit = ethers.utils.parseEther(
      ctx.config.minProfitThreshold
    );
    ...
    if (!minProfit.eq(expectedMinProfit)) {
      throw new Error(
        `Min profit mismatch: expected ${ethers.utils.formatEther(expectedMinProfit)}, got ${ethers.utils.formatEther(minProfit)}`
      );
    }

    if (!maxLoan.eq(expectedMaxLoan)) {
      throw new Error(
        `Max loan mismatch: expected ${ethers.utils.formatEther(expectedMaxLoan)}, got ${ethers.utils.formatEther(maxLoan)}`
      );
    }

    if (recipient.toLowerCase() !== ctx.config.profitRecipient.toLowerCase()) {
      throw new Error(
        `Profit recipient mismatch: expected ${ctx.config.profitRecipient}, got ${recipient}`
      );
    }

    if (manager.toLowerCase() !== ctx.config.positionManager.toLowerCase()) {
      throw new Error(
        `Position manager mismatch: expected ${ctx.config.positionManager}, got ${manager}`
      );
    }
...
function saveDeploymentResult(ctx: DeploymentContext, result: any): void {
  ...
  const outputPath = path.join(
    __dirname,
    `../reports/deployment-${ctx.network}-${Date.now()}.json`
  );
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (ctx.dryRun) {
      ...
      console.log(
        `💡 Estimated deployment cost: ${ethers.utils.formatEther(gasEstimate.estimatedCost)} ETH`
      );
      console.log(
        '🔄 To deploy for real, remove DRY_RUN=true from environment'
      );
      return;
    }
