...
async function demoMultiPool() {
  ...
  console.log('\n🔧 Configuration:');
  console.log(
    `  - Global Profit Threshold: $${process.env.PROFIT_THRESHOLD_USD} USD`
  );
  console.log(
    `  - Pool-specific Thresholds: ${process.env.POOL_PROFIT_THRESHOLD_USD__WETH_USDC_0_05_ ? 'Configured' : 'None'}`
  );
  ...
  if (allMetrics.poolMetrics) {
    for (const [poolId, poolMetrics] of Object.entries(
      allMetrics.poolMetrics
    )) {
      console.log(`  ${poolId}:`);
      console.log(`    - Swaps Detected: ${poolMetrics.swapsDetected}`);
      console.log(
        `    - Success Rate: ${(poolMetrics.successRate * 100).toFixed(1)}%`
      );
