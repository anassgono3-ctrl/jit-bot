...
function validateEnvironment() {
  ...
  console.log(
    `   ETHEREUM_RPC_URL: ${ethereumRpcUrl ? `${ethereumRpcUrl.slice(0, 40)}...` : 'NOT SET'}`
  );
  console.log(
    `   RPC_URL_HTTP: ${rpcUrlHttp ? `${rpcUrlHttp.slice(0, 40)}...` : 'NOT SET'}`
  );
  console.log(
    `   Effective RPC URL: ${finalRpcUrl ? `${finalRpcUrl.slice(0, 40)}...` : 'MISSING'}`
  );

  if (!finalRpcUrl && (network === 'mainnet' || network === 'arbitrum')) {
    errors.push(
      'ETHEREUM_RPC_URL (or RPC_URL_HTTP) is required for mainnet/arbitrum deployment'
    );
  }

  if (!ethereumRpcUrl && rpcUrlHttp) {
    warnings.push(
      'Using RPC_URL_HTTP as fallback. Consider setting ETHEREUM_RPC_URL for consistency'
    );
  }
  ...
  console.log(
    `   MIN_PROFIT_THRESHOLD: ${minProfitThreshold || 'NOT SET (default: 0.01 ETH)'}`
  );
  console.log(
    `   MAX_LOAN_SIZE: ${maxLoanSize || 'NOT SET (default: 1000 ETH)'}`
  );
  ...
  console.log(
    `   PROFIT_RECIPIENT: ${profitRecipient || 'NOT SET (will use deployer address)'}`
  );
  console.log(
    `   POSITION_MANAGER: ${positionManager || 'NOT SET (will use default: 0xC36442b4a4522E871399CD717aBDD847Ab11FE88)'}`
  );
  ...
  if (profitRecipient && profitRecipient.trim() !== '') {
    if (!ethers.utils.isAddress(profitRecipient.trim())) {
      errors.push(
        `PROFIT_RECIPIENT contains invalid address: "${profitRecipient}"`
      );
    }
  }
  ...
  if (positionManager && positionManager.trim() !== '') {
    if (!ethers.utils.isAddress(positionManager.trim())) {
      errors.push(
        `POSITION_MANAGER contains invalid address: "${positionManager}"`
      );
    }
  }
  ...
  console.log(
    `   MIN_REQUIRED_ETH: ${minRequiredEth || 'NOT SET (default: 0.0111)'}`
  );
  ...
  console.log(
    `   ETHERSCAN_API_KEY: ${etherscanApiKey ? mask(etherscanApiKey) : 'NOT SET'}`
  );
  console.log(`   VERIFY_CONTRACTS: ${verifyContracts || 'NOT SET'}`);

  if (verifyContracts === 'true' && !etherscanApiKey) {
    warnings.push(
      'VERIFY_CONTRACTS=true but ETHERSCAN_API_KEY not set - verification will fail'
    );
  }
  ...
  if (errors.length > 0) {
    ...
    if (!dryRun) {
      ...
      console.log(
        '🔥 This configuration will deploy real contracts with real funds'
      );
