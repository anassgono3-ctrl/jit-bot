...
const {
  fastSimulate,
  quickProfitabilityCheck,
} = require('../src/simulator/fastSim');
const {
  runPreflightSimulation,
  forkSimulate,
} = require('../src/simulator/forkSim');
...
const CONFIG = {
  ...
  tolerance: {
    profitErrorThreshold: 0.1, // 10% error threshold
    gasErrorThreshold: 0.2, // 20% gas estimate error threshold
...
class SimulationComparison {
  ...
  calculateComparison(fastResult, forkResult) {
    ...
    try {
      // Profit comparison
      if (
        fastResult.expectedNetProfitUsd !== undefined &&
        forkResult.expectedNetProfitUSD !== undefined
      ) {
        const fastProfit = fastResult.expectedNetProfitUsd;
        const forkProfit = forkResult.expectedNetProfitUSD;

        if (forkProfit !== 0) {
          comparison.profitError =
            Math.abs(fastProfit - forkProfit) / Math.abs(forkProfit);
        }
      }
      ...
      comparison.profitableMismatch =
        fastResult.profitable !== forkResult.profitable;
    } catch (error) {
  ...
  calculateStatistics() {
    ...
    const timings = {
      fastSim: this.results
        .map((r) => r.timings.fastSimMs)
        .filter((t) => t > 0),
      forkSim: this.results
        .map((r) => r.timings.forkSimMs)
        .filter((t) => t > 0),
    };
    ...
    return {
      profitAccuracy: {
        ...
        mean:
          validProfitErrors.reduce((a, b) => a + b, 0) /
            validProfitErrors.length || 0,
      },
      gasAccuracy: {
        ...
        mean:
          validGasErrors.reduce((a, b) => a + b, 0) / validGasErrors.length ||
          0,
      },
      performance: {
        ...
        speedupRatio:
          this.calculatePercentile(timings.forkSim, 0.5) /
          this.calculatePercentile(timings.fastSim, 0.5),
      },
      mismatches: {
        profitabilityMismatches: this.results.filter(
          (r) => r.comparison.profitableMismatch
        ).length,
        totalComparisons: this.results.length,
        mismatchRate:
          this.results.filter((r) => r.comparison.profitableMismatch).length /
          this.results.length,
      },
    };
  }
    ...
    return {
      ...
      accuracy: {
        profit: {
          ...
          passesThreshold:
            stats.profitAccuracy.median < CONFIG.tolerance.profitErrorThreshold,
        },
        gas: {
          medianError: `${(stats.gasAccuracy.median * 100).toFixed(2)}%`,
          p95Error: `${(stats.gasAccuracy.p95 * 100).toFixed(2)}%`,
          passesThreshold:
            stats.gasAccuracy.median < CONFIG.tolerance.gasErrorThreshold,
        },
      },
      performance: {
    ...
    return {
      ...
      fastParams: {
        ...
        swapAmountIn: ethers.BigNumber.from(
          fixture.swapAmount || fixture.amountIn || '1000000000000000000'
        ), // 1 ETH default
      ...
      forkParams: {
        ...
        liquidityAmount: ethers.BigNumber.from(
          fixture.liquidityAmount || '500000000000000000'
        ), // 0.5 ETH
  ...
  try {
    ...
    const fastResult = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('FastSim timeout')),
    ...
    const forkResult = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('ForkSim timeout')),
          CONFIG.simulation.timeoutMs
        )
      ),
    ]);
    ...
    console.warn(
      `Simulation failed for fixture ${fixture.filename}: ${error.message}`
    );
    return null;
  }
}
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < fixtures.length; i++) {
      ...
      console.log(
        `[${i + 1}/${fixtures.length}] Processing ${fixture.filename}...`
      );
      ...
      if (result) {
        ...
        const profitMatch =
          fastResult.profitable === forkResult.profitable ? '✅' : '❌';
        console.log(
          `  ${profitMatch} Fast: ${fastResult.profitable}, Fork: ${forkResult.profitable}`
        );
      } else {
        failed++;
      }
    ...
    console.log(
      `📈 Profit Accuracy - Median: ${report.accuracy.profit.medianError}, P95: ${report.accuracy.profit.p95Error}`
    );
    console.log(
      `⛽ Gas Accuracy - Median: ${report.accuracy.gas.medianError}, P95: ${report.accuracy.gas.p95Error}`
    );
    console.log(
      `⚡ Performance - FastSim: ${report.performance.fastSimMedianMs}ms, ForkSim: ${report.performance.forkSimMedianMs}ms (${report.performance.speedupFactor} speedup)`
    );
    console.log(
      `🎯 Profitability Consistency: ${report.profitabilityConsistency.mismatchRate} mismatch rate`
    );

    // Save detailed report
    const reportPath = path.join(
      __dirname,
      '../reports/fastSim-vs-forkSim-comparison.json'
    );
