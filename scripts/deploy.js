...
function validateEnvironment(network, dryRun) {
  ...
  console.log(
    `   ETHEREUM_RPC_URL: ${ethereumRpcUrl ? `${ethereumRpcUrl.slice(0, 30)}...` : 'not set'}`
  );
  console.log(
    `   RPC_URL_HTTP: ${rpcUrlHttp ? `${rpcUrlHttp.slice(0, 30)}...` : 'not set'}`
  );
  console.log(
    `   Final RPC URL: ${finalRpcUrl ? `${finalRpcUrl.slice(0, 30)}...` : 'MISSING'}`
  );
  ...
  console.log(
    `   POSITION_MANAGER: ${positionManager || 'not set (will use default)'}`
  );
...
async function deploy() {
  ...
  if (!dryRun) {
    ...
    console.log(
      '⚠️  Ensure you have sufficient ETH and understand the risks\n'
    );
  }
  ...
  try {
    ...
    if (!dryRun && !process.env.I_UNDERSTAND_LIVE_RISK) {
      throw new Error(
        'I_UNDERSTAND_LIVE_RISK=true required for live deployment'
      );
    }
