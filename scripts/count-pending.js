...
if (!WS_URL) {
  ...
  console.error(
    'Example: WS_URL=wss://eth-mainnet.ws.alchemyapi.io/v2/YOUR_KEY node scripts/count-pending.js'
  );
  process.exit(1);
}
...
function logCounts(interval) {
  ...
  console.log(
    `[${new Date().toISOString()}] Pending TX count (${interval}): ${count} (total runtime: ${elapsed}s)`
  );
  ...
  ws.on('message', (data) => {
    try {
      ...
      if (message.id === 1 && message.result) {
        ...
        console.log(
          `📡 Subscribed to pending transactions (subscription ID: ${subscriptionId})`
        );
