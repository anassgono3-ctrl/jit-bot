...
async function simulateExample() {
  ...
  try {
    ...
    console.log(
      '   Amount in:',
      ethers.utils.formatEther(exampleSwap.amountIn),
      'ETH'
    );
    console.log('   Pool:', exampleSwap.pool);
    console.log(
      '   Gas price:',
      ethers.utils.formatUnits(exampleSwap.gasPrice, 'gwei'),
      'gwei'
    );
    ...
    console.log(
      '   Amount0:',
      ethers.utils.formatEther(jitParams.amount0),
      'ETH'
    );
    console.log(
      '   Amount1:',
      ethers.utils.formatEther(jitParams.amount1),
      'ETH'
    );

    // Run simulation
    console.log('🔄 Running simulation...');
    const simulationResult = await simulator.simulateJitBundle(
      exampleSwap,
      jitParams
    );
    ...
    console.log(
      '   Profitable:',
      simulationResult.profitable ? '✅ YES' : '❌ NO'
    );
    console.log(
      '   Estimated profit:',
      ethers.utils.formatEther(simulationResult.estimatedProfit),
      'ETH'
    );
    console.log('   Gas used:', simulationResult.gasUsed.toString());
    console.log(
      '   Gas price:',
      ethers.utils.formatUnits(simulationResult.gasPrice, 'gwei'),
      'gwei'
    );
    console.log(
      '   Flash loan fees:',
      ethers.utils.formatEther(simulationResult.flashLoanFees),
      'ETH'
    );
    console.log(
      '   Net profit:',
      ethers.utils.formatEther(simulationResult.netProfit),
      'ETH'
    );
    ...
    const profitMargin = simulationResult.netProfit
      .mul(100)
      .div(exampleSwap.amountIn);
    const costBreakdown = simulationResult.gasUsed.mul(
    ...
    console.log(
      ...
      simulationResult.estimatedProfit.div(simulationResult.gasUsed).toString(),
      'wei'
    );
    ...
    const smallSwap = {
      ...exampleSwap,
      amountIn: ethers.utils.parseEther('5').toString(),
    };
    ...
    console.log(
      '   Small swap (5 ETH):',
      smallResult.profitable ? 'Profitable' : 'Not profitable'
    );

    // Scenario 2: Higher gas price
    const highGasSwap = { ...exampleSwap, gasPrice: '100000000000' }; // 100 gwei
    const highGasResult = await simulator.simulateJitBundle(
      highGasSwap,
      jitParams
    );
    console.log(
      '   High gas (100 gwei):',
      highGasResult.profitable ? 'Profitable' : 'Not profitable'
    );
    ...
    const narrowResult = await simulator.simulateJitBundle(
      exampleSwap,
      narrowJitParams
    );
    console.log(
      '   Narrow tick range:',
      narrowResult.profitable ? 'Profitable' : 'Not profitable'
    );
