...
async function testAlchemyHttp(rpcUrl) {
  ...
  try {
    ...
    if (chainId.chainId !== ETHEREUM_MAINNET_CHAIN_ID) {
      console.log(
        `⚠️  Warning: Expected mainnet (chainId=1), got ${chainId.chainId}`
      );
    }
    ...
    console.log(
      `✅ Latest block: ${latestBlock.number} (${new Date(latestBlock.timestamp * 1000).toISOString()})`
    );
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (response.id === 1 && response.result) {
          ...
          console.log(
            `✅ Latest block via WebSocket: ${parseInt(block.number, 16)} (${block.hash.substring(0, 10)}...)`
          );
          testsPassed++;
          ...
          console.log(
            `📡 New block notification: ${parseInt(block.number, 16)}`
          );
        ...
        console.error(
          `❌ Failed to parse WebSocket message: ${parseError.message}`
        );
      }
    });
...
async function main() {
  ...
  console.log(
    '🔒 Safety: Read-only operations, no transactions will be sent\n'
  );
