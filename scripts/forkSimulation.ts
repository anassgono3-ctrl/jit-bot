  ...
  try {
    ...
    const simulationConfig: SimulationConfig = {
      ...
      blockNumber: process.env.FORK_BLOCK_NUMBER
        ...
        : undefined,
      targetPools: process.env.TARGET_POOLS?.split(',') || [
        'WETH-USDC-0.05%',
        'ETH-USDT-0.3%',
        'WBTC-ETH-0.3%',
      ],
    ...
    const blockNumber =
      simulationConfig.blockNumber || (await getLatestBlockNumber(provider));
    console.log(`📦 Using block number: ${blockNumber}`);

    // Initialize simulation components
    const forkSimulator = new ForkSimulator(
      simulationConfig.rpcUrl,
      blockNumber
    );
    ...
    console.log(
      `💰 Swap sizes: Small=${ethers.utils.formatEther(simulationConfig.swapSizes.small)} ETH, Medium=${ethers.utils.formatEther(simulationConfig.swapSizes.medium)} ETH, Whale=${ethers.utils.formatEther(simulationConfig.swapSizes.whale)} ETH`
    );
    console.log('');
    ...
    for (const poolName of simulationConfig.targetPools) {
      ...
      for (const [sizeLabel, amountIn] of Object.entries(
        simulationConfig.swapSizes
      )) {
        console.log(
          `  📊 Testing ${sizeLabel} swap (${ethers.utils.formatEther(amountIn)} ${poolConfig.symbol0})`
        );
        ...
        try {
          ...
          const netProfitUsdc = result.netProfitUsdc
            ? result.netProfitUsdc.toFixed(2)
            : 'N/A';

          console.log(`     ${profitIcon} Profitable: ${result.profitable}`);
          console.log(
            `     💰 Net Profit: ${netProfitEth} ETH (~$${netProfitUsdc})`
          );
          console.log(`     ⛽ Gas Used: ${result.gasUsed.toLocaleString()}`);
          console.log(
            `     💸 Gas Cost: ${ethers.utils.formatEther(result.gasCostEth)} ETH`
          );
          console.log(
            `     🏦 LP Fees: ${ethers.utils.formatEther(result.lpFeesEth)} ETH`
          );
    ...
    console.log(
      `Profitable scenarios: ${summary.profitableCount} (${((summary.profitableCount / summary.totalSimulations) * 100).toFixed(1)}%)`
    );
    console.log(
      `Total potential profit: ${ethers.utils.formatEther(summary.totalProfitEth)} ETH (~$${summary.totalProfitUsdc.toFixed(2)})`
    );
