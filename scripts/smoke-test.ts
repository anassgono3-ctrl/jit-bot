...
async function runSmokeTest() {
  ...
  try {
    ...
    const result1 = await orchestrator.validateFlashloanParameters(
      usdcToken,
      amount100,
      provider
    );
    ...
    console.log(
      `💰 Fee: ${result1.fee ? utils.formatUnits(result1.fee, 18) : 'N/A'} (engine units)`
    );
    console.log(
      `📊 Amount: 100 USDC (${utils.formatUnits(amount100, 18)} engine units)`
    );
    console.log(
      `📝 Issues: ${result1.issues.length > 0 ? result1.issues.join(', ') : 'None'}`
    );

    console.log(
      '\n📊 Test 2: Large amount (1000 USDC) - should fallback to Aave'
    );
    const result2 = await orchestrator.validateFlashloanParameters(
      usdcToken,
      amount1000,
      provider
    );
    ...
    console.log(
      `💰 Fee: ${result2.fee ? utils.formatUnits(result2.fee, 18) : 'N/A'} (engine units)`
    );
    console.log(
      `📊 Amount: 1000 USDC (${utils.formatUnits(amount1000, 18)} engine units)`
    );
    console.log(
      `📝 Issues: ${result2.issues.length > 0 ? result2.issues.join(', ') : 'None'}`
    );

    console.log('\n📊 Test 3: Tiny amount (0.001 USDC) - should show warnings');
    const result3 = await orchestrator.validateFlashloanParameters(
      usdcToken,
      amount0_001,
      provider
    );
    ...
    console.log(
      `💰 Fee: ${result3.fee ? utils.formatUnits(result3.fee, 18) : 'N/A'} (engine units)`
    );
    console.log(
      `📊 Amount: 0.001 USDC (${utils.formatUnits(amount0_001, 18)} engine units)`
    );
    console.log(
      `📝 Issues: ${result3.issues.length > 0 ? result3.issues.join(', ') : 'None'}`
    );
    console.log(
      `⚠️ Warnings: ${result3.warnings && result3.warnings.length > 0 ? result3.warnings.join(', ') : 'None'}`
    );
    ...
    console.log(
      `💰 Fee: ${utils.formatUnits(selection.fee, 18)} (engine units)`
    );
