...
export function loadConfig(): JitBotConfig {
  ...
  if (useBloxroute) {
    if (!bloxrouteWsUrl) {
      console.warn('⚠️  USE_BLOXROUTE=true but BLOXROUTE_WS_URL is not set');
    }
    if (!bloxrouteAuthHeader) {
      console.warn('⚠️  USE_BLOXROUTE=true but BLOXROUTE_AUTH_HEADER is not set');
    }
  }
